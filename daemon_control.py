...
def main():
    """守护者进程控制脚本主函数"""
    parser = argparse.ArgumentParser(description='HKEX 公告下载器 - 守护者进程控制',
...
使用示例:
  ...
  %(prog)s test                              # 测试配置
    ...
    parser.add_argument('action', choices=['start', 'stop', 'restart', 'status', 'test'], help='要执行的操作')
    ...
    # 构建对应的main.py参数
    main_args = ['--config', args.config, f'--daemon-{args.action}']
