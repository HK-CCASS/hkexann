...
class AsyncHKEXDownloader(HKEXDownloader):
    ...
    async def _take_rest(self):
        ...
        self.logger.info(
            f"🛌 工作了 {work_elapsed / 60:.1f} 分钟，开始休息 {self.rest_duration / 60:.0f} 分钟以避免被封禁...")

        # 创建休息进度条
        rest_pbar = tqdm(total=self.rest_duration, desc="休息中", unit="秒",
    ...
    def async_get_announcement_list(self, stockcode: str, start_date: datetime, end_date: datetime,
    ...
    async def async_download_stock_announcements(self, task: Dict[str, Any]) -> Tuple[str, int]:
        ...
        if download_tasks:
            ...
            # 创建文件级别的进度条
            file_pbar = async_tqdm(total=len(download_tasks), desc=f"下载文件[{stockcode}]", unit="文件", leave=False)

            # 并发执行所有下载任务
            results = await asyncio.gather(*[download_with_progress(task, file_pbar) for task in download_tasks],
