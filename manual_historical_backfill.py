...
class ManualHistoricalBackfillProcessor:
    ...
    async def _resume_state_callback(self, progress_data: Dict[str, Any]):
        ...
        try:
            # 更新进度条（如果存在）
            if self.stock_progress_bar:
                # progress_data['processed_stocks'] 包含所有已处理股票（本次运行）
            ...
            if self.resume_state:
                ...
                parameters = self.resume_state['parameters']
            else:
                # 新任务模式：使用当前参数
                parameters = {
                    'stock_codes': list(self.historical_processor.monitored_stocks),
                    'start_date': None,  # 会在外层设置
                    'end_date': None,
                    'batch_size': self.historical_processor.stock_batch_size,
                    'enable_filtering': True
                }
            
            # 实时保存状态
            await self.save_resume_state(parameters, progress_data)
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.dual_filter = self.hkex_filter  # 设置dual_filter引用
            ...
            self.historical_processor = CorrectedHistoricalProcessor(
                ...
                dual_filter=self.hkex_filter,
                vectorizer=self.vector_processor,
                monitored_stocks=set(),  # 动态设置
                config=historical_config,
                progress_callback=self._resume_state_callback
            )
    ...
    async def process_stocks_historical(self,
        ...
        try:
            ...
            # 🔧 修复：区分原始股票列表和当前要处理的股票列表
            # stock_codes 在续传模式下已经是剩余股票列表
            current_stock_codes = stock_codes  # 当前要处理的股票（可能是剩余的）
            original_stock_codes = resume_state['parameters']['stock_codes'] if resume_state else stock_codes
            
            self.stats['total_stocks'] = len(current_stock_codes)
            ...
            try:
                ...
                if not self.resume_state:
                    ...
                    await self.save_resume_state({
                        'stock_codes': original_stock_codes,
                ...
                with tqdm(total=total_stocks_for_progress, desc="📊 处理股票", unit="只",
                    ...
                    # 确保进度条显示完成状态
                    if self.stock_progress_bar.n < total_stocks_for_progress:
                        self.stock_progress_bar.update(total_stocks_for_progress - self.stock_progress_bar.n)
